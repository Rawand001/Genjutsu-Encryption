...
class EncryptionApp(QWidget):
    ...
    def send_message(self, sender, recipient_public_key, recipient_private_key):
        ...
        hashed_actual_message = hash_message(actual_message)
        ...
        while retries < max_retries:
            try:
                cursor.execute('INSERT INTO messages (actual_msg, generated_msg) VALUES (?, ?)', (hashed_actual_message, hashed_generated_message))
                conn.commit()
                break
        ...
        if actual_message_row:
            ...
            if sender == "User 1":
                self.user2_display.append(f"<b>User 2 decrypted:</b> {actual_message}<br><br>")
            else:
                self.user1_display.append(f"<b>User 1 decrypted:</b> {actual_message}<br><br>")
        else:
            if sender == "User 1":
